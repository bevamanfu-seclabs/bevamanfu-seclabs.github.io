...
| Peer IP(WAN) |    172.28.28.106 | 172.28.28.122   |
| LAN Subnet   | 172.16.3.0/24   | 10.81.3.0/24  |
| LAN Gateway    |   172.16.3.1   |  10.81.3.2   |
| Endpoint ( LAN)   |  Ubuntu - 172.16.3.3  | Kali linux - 10.81.3.3 |
...
<div style="text-align:center;">
 ...
 The lab topology consisted of two FortiGate VMs connected through a shared transit network (*172.28.28.0/24*) simulating a WAN segment. Linux endpoints were placed behind each FortiGate to simulate LAN hosts and serve as ping test points for tunnel validation. 
...
It is good practice to use the Route monitor in FortiGate to verify that the remote IP address is not already in use in your local network and to identify the interface associated with the local IP address. 

#### *1. Create Custom IPSec Tunnel*
The tunnel was created via **VPN → VPN Tunnels → Create New → Custom** with the Phase 1 and Phase 2 parameters  listed above.
...
<div style="text-align:center;">
  <img src="{{ '/assets/projects/VPN/vpn-2.jpeg' | relative_url }}" 
      alt="Network Configuration" 
      style="width:500px; height:300px; object-fit:contain; max-width:100%;">
       <p style="font-size:0.9em; color:#666; margin-top:10px;">
...
A firewall policy was created on Site A to allow traffic from Site A to Site B though the tunnel. NAT was disabled and security profiles including AntiVirus and IPS were applied. Logging of security events was enabled.
...
Another policy was created to allow traffic from Site B to Site A, thus allowing bi-directional flow of traffic across the tunnel. 
...
Following the initial configuration, a deliberate troubleshooting exercise where multiple misconfigurations were introduced on both FortiGate instances was conducted. The task was to identify, diagnose, and resolve each issue. 
...
**3. NAT enabled on firewall policy**
  ...
  With NAT enabled, packets from LAN_A would have their source IP translated to Site A's WAN IP before entering the tunnel. Since Site B's firewall policy expects source addresses from *172.16.3.0/24*, the translated WAN IP (*172.28.28.0/24*) would not match details in the Phase 2 parameters and traffic would be dropped. 
...
**6. NAT enabled on Site B policy**
  ...
  Just like on Site A, Site B also had NAT enabled on the inbound VPN policy.

  <div style="text-align:center;">
  <img src="{{ '/assets/projects/VPN/ts-NATB.png' | relative_url }}" 
        style="width:700px; height:100px; object-fit:contain; max-width:100%;">
  </div> 
...
**8. Change in IKE version**
  ...
  *Solution* :The correct IKE version was used.
<br>
